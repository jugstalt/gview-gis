...
{
    public class SqlFDBNetworkFeatureclass : IFeatureClass, INetworkFeatureClass
    {
        private SqlFDB _fdb;
        ...
        private class SqlFDBGraphTableAdapter : IGraphTableAdapter
        {
            private string _name, _connString;
            private SqlFDBNetworkFeatureclass _nfc;
        ...
        {
            return new SqlFDBGraphTableAdapter(_fdb, this);
        }
        ...
        public IFeature GetNodeFeature(int nid)
        {
            ...
            filter.WhereClause = "FDB_OID=" + nid;
            filter.AddField("*");
        ...
        public int MaxNodeId
        {
            get
            {
                try
                {
                    using (SqlConnection connection = new SqlConnection(_fdb.ConnectionString))
                    {
                        SqlCommand command = new SqlCommand("SELECT MAX([FDB_OID]) FROM " + _fdb.TableName("FC_" + _nodeFc.Name), connection);
                        connection.Open();
        ...
        public bool HasDisabledSwitches
        {
            get
            {
                try
                {
                    using (SqlConnection connection = new SqlConnection(_fdb.ConnectionString))
                    {
                        SqlCommand command = new SqlCommand("SELECT COUNT([FDB_OID]) FROM " + _fdb.TableName("FC_" + _nodeFc.Name) + " WHERE [SWITCH]=1 AND [STATE]=0", connection);
                        connection.Open();
