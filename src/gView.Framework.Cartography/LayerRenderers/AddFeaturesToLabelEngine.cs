...
{
    public sealed class AddFeaturesToLabelEngine
    {
        private Map _map;
        ...
        public AddFeaturesToLabelEngine(
                           Map map,
        ...
        async public Task AddFeatures()
        {
            try
            {
                if (_layer == null || !_layer.FeatureLabelPriority.HasValue || _map == null)
                {
                    return;
                }
            ...
            catch (Exception ex)
            {
                if (_map is IServiceMap && ((IServiceMap)_map).MapServer != null)
                {
                    await ((IServiceMap)_map).MapServer.LogAsync(
                        ...
                        "AddFeaturesToLabelEngine:" + (_layer != null ? _layer.Title : string.Empty),
                        loggingMethod.error,
                        ex.Message + "\n" + ex.Source + "\n" + ex.StackTrace);
                }
                if (_map != null)
                {
                    if (_map != null)
                    {
                        _map.AddRequestException(new Exception("AddFeaturesToLabelEngine: " + (_layer != null ? _layer.Title : string.Empty) + "\n" + ex.Message, ex));
                    }
                }
            }
            finally
