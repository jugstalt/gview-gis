...
public class MapRenderService : IDisposable
{
    ...
    public void InitMap(Map map, ISpatialReference? mapControlSRef = null)
    {
        ...
        {
            bounds = _geoTransformer.Transform(bounds, sRef, _map.SpatialReference, _map.Display?.DatumTransformations)?.Envelope;
    ...
    public void SetBoundsAndSize(IEnvelope bounds, int imageWidth, int imageHeight)
    {
        ...
        {
            _map.Display.ZoomTo(_geoTransformer.Transform(bounds, _mapControlSRef, _map.SpatialReference, _map.Display?.DatumTransformations)?.Envelope);
        }
    }
