...
namespace gView.DataSources.GDAL
{
    public class RasterClassV3 : IRasterClass, IRasterFile, IRasterFileBitmap, IPointIdentify, IGridIdentify, IGridClass, IPersistable, IDisposable
    {
        ...
        public RasterClassV3(IRasterDataset dataset, string filename)
            : this(dataset, filename, null)
        {
        }
        public RasterClassV3(IRasterDataset dataset, string filename, IPolygon polygon)
        {
            try
            {
        ...
        public float GridQuery(IDisplay display, IPoint point, ISpatialReference sRef)
        {
            ...
            if (this.SpatialReference != null && sRef != null &&
                !sRef.Equals(this.SpatialReference))
            {
                point = GeometricTransformerFactory.Transform2D(point, sRef, this.SpatialReference, display) as IPoint;
            }
            if (point == null)
            {
                return floatNodata;
