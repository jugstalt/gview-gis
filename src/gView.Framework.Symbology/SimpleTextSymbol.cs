...
namespace gView.Framework.Symbology
{
                                    ...
                                    IFontSymbol
    {
                    ...
                    while (true)
                    {
                        ...
                        switch (format.LineAlignment)
                        {
                            case StringAlignment.Near:
                                polygon = new AnnotationPolygon((float)x + .1f * cSize.Width, (float)y + .2f * cSize.Height,
                                                                           .8f * cSize.Width, .6f * cSize.Height);
                                break;
                            case StringAlignment.Far:
                                polygon = new AnnotationPolygon((float)x + .1f * cSize.Width, (float)y - .8f * cSize.Height,
                                                                           .8f * cSize.Width, .6f * cSize.Height);
                                break;
                            default:
                                polygon = new AnnotationPolygon((float)x + .1f * cSize.Width, (float)y - .6f * cSize.Height / 2f,
                                                                           .8f * cSize.Width, .6f * cSize.Height);
                                break;
                        }
        ...
        private AnnotationPolygon AnnotationPolygon(IDisplay display, CanvasSizeF size, float x, float y, TextSymbolAlignment alignment)
        {
            ...
            #region test
            if (size.Width < SymbolSpacing)
            {
                float widthDiff = SymbolSpacing - size.Width;
                x1 -= widthDiff / 2f; size.Width = SymbolSpacing;
            }
            if (size.Height < SymbolSpacing)
            {
                float heightDiff = SymbolSpacing - size.Height;
                y1 -= heightDiff / 2f; size.Height = SymbolSpacing;
            }
            #endregion
        ...
        new virtual public void Load(IPersistStream stream)
        {
            ...
            this.SymbolSpacing = (float)stream.Load("symbolspacing", 0f);
        ...
        new virtual public void Save(IPersistStream stream)
        {
            ...
            stream.Save("symbolspacing", this.SymbolSpacing);
        ...
        public override object Clone()
        {
            ...
            tSym.SymbolSpacing = this.SymbolSpacing;
