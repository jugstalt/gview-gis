...
namespace gView.DataSources.Fdb.PostgreSql.Cursors
{
    ...
    /// geometry post-filter is active (<c>sFilter != null</c>) <c>limit</c>/<c>offset</c> are
    /// enforced here, after the geometry check.
    /// </summary>
    internal sealed class PgSpatialFeatureCursor : PgFeatureCursorBase
    {
        ...
        private PgSpatialFeatureCursor(
        ...
        public static async Task<IFeatureCursor> Create(
            ...
            IGeometryDef geomDef, ISpatialReference toSRef, IDatumTransformations datumTransformations)
        {
            var cursor = new PgSpatialFeatureCursor(spatialFilter, limit, beginRecord, geomDef, toSRef, datumTransformations);
            ...
            var builder = new PostgreSqlSelectBuilder(sql)
