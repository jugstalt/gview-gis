    ...
    public class ArcIMSController : BaseController
    {
        ...
        async public Task<IActionResult> EsriMap(string cmd, string ServiceName, string content)
        {
            return await SecureMethodHandler(async (identity) =>
            {
                if (cmd == "ping")
                {
                    return Result("gView MapServer Instance v" + SystemVariables.gViewVersion.ToString(), "text/plain");
                }
                if (cmd == "getversion")
                {
                    return Result(SystemVariables.gViewVersion.ToString(), "text/plain");
                }
                if (cmd == "capabilities")
                {
