    ...
    public sealed class Polygon : IPolygon, ITopologicalOperation
    {
        ...
        public void VerifyHoles()
        {
            ...
            foreach (IRing ring in v)
            {
                ...
                {
                    if (GeoProcessing.Algorithm.Jordan(P, ring))
                    {
                        hole = !(P is IHole);
                    }
                }
        ...
        public IEnumerable<IHole> InnerRings(IRing ring)
        {
            ...
            {
                if (GeoProcessing.Algorithm.Jordan(ring, hole))
                {
                    result.Add(hole);
                }
            }
        ...
        {
            VerifyHoles();
            IPolygon buffer = Algorithm.PolygonBuffer(this, distance);
            return buffer;
        }
        ...
        {
            VerifyHoles();
            IGeometry polygon = GeoProcessing.Clip.PerformClip(clipper, this);
        ...
        {
            VerifyHoles();
            IGeometry polygon = GeoProcessing.Clip.PerformClip(geometry, this);
            if (!(polygon is IPolygon))
            {
                polygon = null;
            }
