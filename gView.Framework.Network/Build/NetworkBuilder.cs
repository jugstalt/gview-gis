    ...
    public class NetworkBuilder
    {
        ...
        public bool SplitToComplexEdges(NetworkNode node)
        {
            ...
            foreach (NetworkEdge edge in edges)
            {
                ...
                IPoint snapped = Algorithm.Point2PathDistance(edge.Path, node.Point, out dist, out stat);
                if (snapped != null)
                {
                    if (dist <= _tolerance)
                    {
                        Polyline pLine = new Polyline(edge.Path);
                        Polyline p1 = Algorithm.PolylineSplit(pLine, 0, stat);
                        Polyline p2 = Algorithm.PolylineSplit(pLine, stat, double.MaxValue);
                        if (p1.PathCount != 1 ||
                           p2.PathCount != 1)
                        {
                            continue;
